...
fn eval_lexpr(lexpr: &LeftExpression, mem: &Memory) -> Result<Value, String> {
    ...
    use Value::*;
    match lexpr {
        Identifier(id) => match mem.stack.get(id) { // if this is a simply variable, we look on the stack for searching the vlaue
                Some(v) => Ok(v.clone()),
                None => Err(format!("unbound {}",id)),
            },
        Star(lexpr2) => { // otherwise we search on the heap 
